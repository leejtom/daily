...
- (instancetype)init
{
    self = [super init];
    if (self) {
        _position = PLVLCPlayerLogoPositionRightUp;
        _logoAlpha = 1.0;
        _xOffsetScale = 0.03;
        _yOffsetScale = 0.06;
    }
...
@property (nonatomic, strong) NSMutableArray <PLVLCPlayerLogoParam *> *logoParams;
...
- (void)layoutSubviews {
    for (int i = 0; i < [self.logos count]; i++) {
        ...
        PLVLCPlayerLogoParam *param = self.logoParams[i];
...
- (void)insertLogoWithParam:(PLVLCPlayerLogoParam *)param {
    ...
    if (param.position == PLVLCPlayerLogoPositionNone) {
        return;
    }
...
/// 计算 logo frame 值
- (CGRect)getLogoRectWithParam:(PLVLCPlayerLogoParam *)param imageSize:(CGSize)imageSize {
...
/// 计算 logo 大小
- (CGSize)getLogoSizeWithParam:(PLVLCPlayerLogoParam *)param imageSize:(CGSize)imageSize {
